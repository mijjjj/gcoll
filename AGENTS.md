...
- 本地验证方式遵循 `docs/ai/本地验证指令.md`：默认只运行后端测试和前端构建，不启动前后端服务。
...
- 内置 Modbus TCP 南向插件固定使用 `com.gcoll.modbus-tcp`，配置和点位表由宿主保存并下发；插件目录不得保存最终运行配置。
- Modbus TCP 点位读写必须显式区分 `read` 与 `write`，只读区 `discrete_input`、`input_register` 不得写入；读取点位必须按区域和地址排序并批量读取。
