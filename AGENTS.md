...
- 除非用户明确要求兼容旧版本、保留旧接口或保留旧行为，否则修改程序时一切以当前最新需求为准，不必为了仓库内部旧代码保留兼容层、兜底逻辑或双写流程。
- 架构设计、模块边界、内部接口和性能关键路径优先选择最优方案；必要时可以直接替换旧架构、旧字段和旧内部接口，而不是围绕旧实现继续打补丁。
...
- 涉及重构、性能优化、内部接口替换、架构调整或删除旧实现时，同时使用 `.codex/skills/gcoll-latest-first`。
...
- `.codex/skills/gcoll-latest-first`：进行重构、架构替换、性能优化或内部接口清理时使用，默认按最新需求直接推进，不保留旧内部兼容层。
