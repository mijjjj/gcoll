...
内置 Modbus TCP 的协议扩展字段独立保存在 `modbus_tcp_device_profiles`、`modbus_tcp_point_profiles` 和有限窗口调试表 `modbus_tcp_debug_logs`。平台通用设备档案和点位表仍以 `devices`、`device_points` 为准，插件扩展表不得替代通用点位表。
