...
内置 Modbus TCP 的设备级协议配置保存在 `modbus_tcp_device_profiles`，点位协议扩展字段直接保存在通用点位表 `device_points.metadata_json`，有限窗口调试数据保存在 `modbus_tcp_debug_logs`。平台通用设备档案和点位表仍以 `devices`、`device_points` 为准，插件不得引入额外点位表替代通用点位表。
